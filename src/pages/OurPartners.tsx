...
import { Lock, Trash2, ExternalLink, CreditCard as EditIcon } from 'lucide-react';
...
export default function OurPartners() {
  ...
  useEffect(() => {
    const savedPartners = localStorage.getItem('rcf_partners');
    if (savedPartners) {
  ...
  const savePartners = (newPartners: Partner[]) => {
    localStorage.setItem('rcf_partners', JSON.stringify(newPartners));
  ...
  const handleAddPartner = () => {
    if (!formData.name || !formData.description) {
      alert('Please fill in partner name and description');
      return;
    }
  ...
  const handleSaveEdit = () => {
    ...
    savePartners(partners.map(p => p.id === editingPartner!.id ? updatedPartner : p));
    setShowAddForm(false);
