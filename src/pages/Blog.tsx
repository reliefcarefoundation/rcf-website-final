...
import { Lock, Trash2, CreditCard as Edit } from 'lucide-react';
...
export default function Blog() {
  ...
  useEffect(() => {
    const savedPosts = localStorage.getItem('rcf_blog_posts');
    if (savedPosts) {
  ...
  const savePosts = (newPosts: BlogPost[]) => {
    localStorage.setItem('rcf_blog_posts', JSON.stringify(newPosts));
    setPosts(newPosts);
  };
  ...
  const handleSaveEdit = () => {
    ...
    savePosts(posts.map(p => p.id === editingPost!.id ? updatedPost : p));
    setShowAddForm(false);
    setEditingPost(null);
    setFormData({ title: '', text: '', date: '' });
    setImageUrls([]);
  ...
  return (
    <div className="bg-slate-50 min-h-screen py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {sortedPosts.length === 0 && !adminUnlocked ? (
              ...
              {!showAddForm ? (
                ...
                <div className="space-y-4 mt-4">
                  ...
                  <textarea
                    ...
                    rows={12}
                    className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#00529b] bg-white w-full resize-y"
                  />
                  ...
                  <div className="flex space-x-4">
                    <button
                      onClick={editingPost ? handleSaveEdit : handlePublishPost}
